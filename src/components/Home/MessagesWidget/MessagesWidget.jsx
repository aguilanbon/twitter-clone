...
function MessagesWidget() {
  ...
  return (
    <div className={`fixed bg-black xl:w-96 w-80 ${isExpanded ? `h-3/5` : `h-12 items-center`}  bottom-0 right-5 rounded-t-2xl shadow-twitter shadow-white hidden lg:flex cursor-pointer flex-col  transition-all duration-300`}>
        <div className='flex flex-row h-9 w-full justify-between mb-3'>
          <div className='flex h-full w-full pt-2 pl-3'>
            <h1 className='font-bold text-twitterwhite text-lg w-full'  onClick={() => setIsExpanded(prev => !prev)}>Messages</h1>
          </div>
          <div className='flex h-9 mr-3 mt-2'>
            <div className='hover:bg-twitterbggray hover:bg-opacity-80 p-2 rounded-full' onClick={() => setModalIsOpen(true)}>
